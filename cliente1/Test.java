...
class Test {
    ...
    static private boolean doWrite(Scanner ent) throws RemoteException {
        ...
        try {
            System.out
                    .println("Introduzca ID de fichero, cantidad a escribir y carácter que se escribirá en el fichero");
            if (!ent.hasNextLine())
                return false;
            String lin = ent.nextLine();
            Scanner s = new Scanner(lin);
        ...
        try {
            ...
            while (true) {
                ...
                if (s.hasNextInt()) {
                    ...
                    switch (op) {
                    case 0:
                        formatoOK = doOpen(ent);
                        break;
                    case 1:
                        formatoOK = doRead(ent);
                        break;
                    case 2:
                        formatoOK = doWrite(ent);
                        break;
                    case 3:
                        formatoOK = doSeek(ent);
                        break;
                    case 4:
                        formatoOK = doSetLength(ent);
                        break;
                    case 5:
                        formatoOK = doClose(ent);
                        break;
                    }
                }
                if (!formatoOK)
